# CNVS Protocol Engineering Tests
...
This folder contains the **Executable Demonstrative & Engineering Tests (Tests 13, 14, and 15)** for the CNVS protocol.

These tests are separate from the academic MTC suite. Their purpose is to evaluate whether core CNVS execution patterns can be migrated from simplified academic simulations toward more production-oriented engineering constraints.
...
The protocol engineering tests are different. They are designed to evaluate implementation hardening.

For this reason, protocol engineering tests may use non-reproducible cryptographic randomness, Python arbitrary-precision integers, 256-bit finite-field moduli, CSPRNG-based secret generation, and randomized or adaptive adversarial sampling. This makes them less convenient for deterministic academic reproduction, but more relevant for engineering review.

---

## 1. The non-uniform distribution of the weight of invariant constraints
...
The topological multiplier adds a further layer of epistemic isolation. Instead of shattering only the target drinking glass, the system simultaneously shatters additional unrelated objects, such as a glass ashtray, a beer bottle, a picture frame, or other structurally similar but semantically unrelated objects. All resulting shards are then mixed into the same distributed validation environment.

A compromised peripheral verifier may intercept a fragment and correctly measure its local properties (angle, thickness, curvature, local edge compatibility, admissible field value, syntactic validity). However, the verifier cannot determine whether that fragment is a critical load-bearing piece of the original drinking glass, or merely a non-critical shard of structural camouflage.
...
This is the engineering meaning of non-uniform criticality. The system does not merely hide values. It hides the structural relevance of values.

---

## 3. Security Interpretation

The topological multiplier increases the adversary’s uncertainty in two distinct ways. First, it increases the number of fragments that must be observed, classified, or controlled before reconstruction can be attempted. Second, it prevents the adversary from knowing which fragments actually matter for the hidden invariant binding.
...
This ensures that increasing `m` does not merely increase the number of critical fragments. It also expands the surrounding fragment universe. The result is a more realistic engineering model in which the adversary does not observe a clean set of only critical fragments. Instead, the adversary operates inside a larger mixed topology containing both critical and non-critical fragments.

---

## Important Limitation

These tests are still not production CNVS node implementations. They do not include:
